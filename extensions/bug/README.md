...
A three-step bug triage workflow for Spec Kit: assess, fix, and validate. Each bug lives in its own directory under `bugs/<slug>/`, with one Markdown report per stage.
...
|---------|-------------|--------|
| `speckit.bug.assess` | Triages a bug report (pasted text or URL) against the codebase. | `bugs/<slug>/assessment.md` |
| `speckit.bug.fix` | Applies the remediation from the assessment. | `bugs/<slug>/fix.md` |
| `speckit.bug.test` | Validates the fix and records the verification report. | `bugs/<slug>/test.md` |

## Slug Conventions

A *slug* is the per-bug directory name under `bugs/`. It is the only handle the three commands share.
...
- **Automated**: when no human is available to answer, the agent generates a slug itself. The generated slug **MUST** produce a unique directory — if `bugs/<slug>/` already exists, the agent appends the shortest disambiguating suffix needed (`-2`, `-3`, …) or a short date (`-20260605`). Existing bug directories are never overwritten.
...
- `speckit.bug.assess` and `speckit.bug.test` **never modify source code**. They read the repository and write only inside `bugs/<slug>/`.
